...
Future<File> downloadFileWithRetry(
  ...
  try {
    ...
    if (retries > 0 &&
      ...
      await Future<void>.delayed(const Duration(seconds: _retryDelaySeconds));
...
class AppsProvider with ChangeNotifier {
  ...
  void scheduleAutoExport() {
    ...
    _autoExportDebounce = Timer(const Duration(seconds: 2), () {
      if (!_disposed) {
        export(isAuto: true).catchError((Object e) {
...
Future<void> bgUpdateCheck(
  ...
  final List<MapEntry<String, int>> toCheck = <MapEntry<String, int>>[
    ...((params['toCheck'] as Iterable<dynamic>?)
            ?.map(
              (dynamic entry) => MapEntry<String, int>(
                (entry as Map<dynamic, dynamic>)['key'] as String,
                entry['value'] as int,
              ),
            )
            .toList() ??
