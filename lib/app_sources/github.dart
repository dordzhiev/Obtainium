...
class GitHub extends AppSource {
  ...
  List<GeneratedFormItem> get sourceConfigSettingFormItems => [
    ...
    GeneratedFormSwitch('checkRepoRename', label: tr('repoRenamedCheck')),
  ];
        ...
        if (res.statusCode == 200) {
          try {
            ...
            final trimmedLines = utf8
                .decode(
                  base64.decode(body['content'].toString().split('\n').join()),
                )
                .split('\n')
                .map((e) => e.trim());
  ...
  Future<void> checkForRepositoryRename(
    ...
    if (res.statusCode >= 300 && res.statusCode < 400) {
      ...
      if (location != null) {
        ...
        String? newUrl;
        try {
          newUrl = jsonDecode(res2.body)['html_url'] as String?;
        } catch (e) {
          AppLogger.info(
  ...
  List<dynamic> _findReleaseAssetUrls(
    ...
    Map<String, String> sourceConfigSettingValues,
      ...
      (release['assets'] as List<dynamic>?)?.map((e) {
        ...
        final String? url = rawUrl == null
            ? null
            : undoGHProxyMod(rawUrl, sourceConfigSettingValues);
  ...
  void _sortGitHubReleases(
    ...
    releases.sort((a, b) {
      ...
      if (sortMethod != 'name' && stdFormats.isNotEmpty) {
        ...
        final matchA = reg.firstMatch(nameA as String);
        final matchB = reg.firstMatch(nameB as String);
      ...
      return compareAlphaNumeric(nameA.toString(), nameB.toString());
    });
  }
  ...
  Future<APKDetails> _fetchReleaseDetails(
    ...
    if (verifyLatestTag) {
      ...
      final latestUrl = uri.replace(path: '${uri.path}/latest');
      final Response res = await sourceRequest(
        latestUrl.toString(),
        additionalSettings,
      );
  ...
  Future<Map<String, List<String>>> searchCommon(
    ...
    if (res.statusCode == 200) {
      ...
      for (var e in (jsonDecode(res.body)[rootProp] as List<dynamic>)) {
        if ((e['stargazers_count'] ?? e['stars_count'] ?? 0) as int >=
            minStarCount) {
